...
resource "azurerm_virtual_network" "sec-vn" {
  name                = "tfc-network"
  resource_group_name = azurerm_resource_group.rg-tfc.name
  location            = azurerm_resource_group.rg-tfc.location
  ...
  name                 = "tfc1-subnet"
  resource_group_name  = azurerm_resource_group.tfc-rg.name
